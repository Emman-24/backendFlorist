...
class GlobalExceptionHandler {
    ...
    fun handleFloralArrangementNotFound(
        ex: FloralArrangementNotFoundException, request: HttpServletRequest
    ...
    fun handleSlugNotFound(
        ex: FloralArrangementSlugNotFoundException, request: HttpServletRequest
    ): ResponseEntity<ApiResponse.Error> {
    ...
    fun handleSeoNameNotFound(
        ex: FloralArrangementSeoNameNotFoundException, request: HttpServletRequest
    ): ResponseEntity<ApiResponse.Error> {
    ...
    fun handleIllegalState(
        ...
        return ResponseEntity.status(HttpStatus.CONFLICT).body(
                ApiResponse.Error(
                    message = ex.message ?: "Operation not allowed in current state", code = "ILLEGAL_STATE"
                )
            )
    }
    ...
    fun handleMethodArgumentNotValid(
        ...
        logger.warn(
            "Validation failed: uri={}, fields={}", request.requestURI, fieldErrors.keys.joinToString()
        )
        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(
                ApiResponse.ValidationError(
                    message = "Validation failed. Check the highlighted fields.", fieldErrors = fieldErrors
                )
            )
    }
    ...
    fun handleConstraintViolation(
        ...
        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(
                ApiResponse.ValidationError(
                    message = "Request parameters are invalid.", fieldErrors = fieldErrors
                )
            )
    }
    ...
    fun handleMissingParam(
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
                ApiResponse.Error(
                    message = "Required parameter '${ex.parameterName}' is missing.", code = "MISSING_PARAMETER"
                )
            )
    }
    ...
    fun handleTypeMismatch(
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
                ApiResponse.Error(
                    message = "Parameter '${ex.name}' has an invalid value: '${ex.value}'.", code = "TYPE_MISMATCH"
                )
            )
    }
    ...
    fun handleMediaTypeNotSupported(
        ...
        return ResponseEntity.status(HttpStatus.UNSUPPORTED_MEDIA_TYPE).body(
                ApiResponse.Error(
                    message = "Media type '${ex.contentType}' is not supported.", code = "UNSUPPORTED_MEDIA_TYPE"
                )
            )
    }
    ...
    fun handleDataIntegrity(
        ...
        val message = when {
            ex.message?.contains("slug", ignoreCase = true) == true -> "A resource with this slug already exists."

            ex.message?.contains("email", ignoreCase = true) == true -> "This email address is already registered."

            ex.message?.contains("username", ignoreCase = true) == true -> "This username is already taken."

            ex.message?.contains("public_id", ignoreCase = true) == true -> "This image has already been uploaded."
    ...
    fun handleMaxUploadSize(
        ...
        return ResponseEntity.status(HttpStatus.PAYLOAD_TOO_LARGE).body(
                ApiResponse.Error(
                    message = "Uploaded file exceeds the maximum allowed size.", code = "FILE_TOO_LARGE"
                )
            )
    }
    ...
    fun handleGeneric(
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
                ApiResponse.Error(
                    message = "An unexpected error occurred. Please try again later.", code = "INTERNAL_ERROR"
                )
            )
    }
    ...
    fun handleNoResourceFound(
        ex: NoResourceFoundException, request: HttpServletRequest
    ): ResponseEntity<ApiResponse.Error> {
    ...
    fun handleFlowerNotFound(
        ex: FlowerNotFoundException, request: HttpServletRequest
    ): ResponseEntity<ApiResponse.Error> {
    ...
    fun handleFaqNotFound(
        ex: FaqNotFoundException, request: HttpServletRequest
    ): ResponseEntity<ApiResponse.Error> {
        logger.warn("FAQ not found: uri={}, message={}", request.requestURI, ex.message)
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body(ApiResponse.Error(message = ex.message ?: "FAQ not found", code = "FAQ_NOT_FOUND"))
    }
